...
class ChangesInEquityReportPage extends Page implements HasTable, HasForms
{
    ...
    public function table(Table $table): Table
    {
        ...
        foreach ($rows as $row) {
            $unionQueries[] = DB::query()->selectRaw('? as id, ? as date, ? as account_code, ? as account_name, ? as description, ? as movement', [
                $index++,
                $row['date'] ?? '',
                $row['account_code'] ?? '',
                $row['account_name'] ?? '',
        ...
        if ($unionQuery === null) {
            $unionQuery = DB::query()->selectRaw('0 as id, NULL as date, NULL as account_code, NULL as account_name, NULL as description, 0 as movement');
        }
