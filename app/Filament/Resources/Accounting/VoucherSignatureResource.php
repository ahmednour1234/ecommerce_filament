...
class VoucherSignatureResource extends Resource
{
    ...
    public static function form(Form $form): Form
    {
        return $form
            ->schema([
                Forms\Components\Section::make(trans_dash('vouchers.signatures.basic_info', 'Basic Information'))
                    ->schema([
                        Forms\Components\TextInput::make('name')
                            ->label(trans_dash('vouchers.signatures.name', 'Name'))
                            ->required()
                            ->maxLength(255)
                            ->helperText(trans_dash('vouchers.signatures.name_helper', 'Display name for the signature (e.g., "Ahmed Nour")')),

                        Forms\Components\TextInput::make('title')
                            ->label(trans_dash('vouchers.signatures.title', 'Title'))
                            ->maxLength(255)
                            ->nullable()
                            ->helperText(trans_dash('vouchers.signatures.title_helper', 'Role or title (e.g., "Accountant", "Manager")')),
                        ...
                        Forms\Components\Select::make('type')
                            ...
                            ->nullable()
                            ->helperText(trans_dash('vouchers.signatures.type_helper', 'Restrict signature to specific voucher types. Leave empty for both.')),

                        Forms\Components\TextInput::make('sort_order')
                            ->label(trans_dash('vouchers.signatures.sort_order', 'Sort Order'))
                            ->numeric()
                            ->default(0)
                            ->helperText(trans_dash('vouchers.signatures.sort_order_helper', 'Lower numbers appear first in selection lists')),
                ...
                Forms\Components\Section::make(trans_dash('vouchers.signatures.signature_image', 'Signature Image'))
                    ->schema([
                        Forms\Components\FileUpload::make('image_path')
                            ...
                            ->nullable()
                            ->helperText(trans_dash('vouchers.signatures.image_helper', 'Upload signature image or stamp. Recommended size: 200x100px'))
                            ->columnSpanFull()
                            ->afterStateUpdated(function ($state, $record) {
                                // Delete old image if new one is uploaded
                                if ($record && $record->image_path && $state !== $record->image_path) {
                        ...
                        Forms\Components\ViewField::make('image_preview')
                            ...
                            ->visible(fn ($record) => $record && $record->image_path)
                            ->columnSpanFull(),
                    ]),
            ]);
    }
            ...
            ->columns([
                ...
                Tables\Columns\BadgeColumn::make('type')
                    ...
                    ->formatStateUsing(fn (?string $state): string => match($state) {
                        ...
                        null => trans_dash('vouchers.signatures.type_both', 'Both'),
                        default => $state,
                    })
                    ->colors([
                        'success' => 'both',
